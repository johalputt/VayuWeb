...
entry carries the rule `id` plus the exact `what`/`why`/`fix` strings a finding renders.
Verified from the registry side by `registry/src/rules.test.ts`, which independently states the
expected id set — so a rule added or removed without acknowledgment on BOTH sides fails CI twice.
Regenerated by the same CI step as `client-built.json`; drift fails the build. The ids are the
contract: findings reference them, the fixer matches on them, and anything that consumes
doctor output in another language compiles against this file rather than against folklore.
