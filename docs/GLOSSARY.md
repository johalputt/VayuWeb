...
Every term the specifications use, defined once. Where a word has a specific WebX meaning that
...
effect. Every Standards Track WXIP must carry one; this is what makes retroactive rule changes
...
removed. WebX's registry is one. The consequence people underestimate: it cannot forget.

**Argon2id** — The memory-hard password-hashing function (RFC 9106) used for WebX's
...
any other peers are known. WebX treats a default bootstrap list as a centralisation risk to be
...
the browsing proxy. It binds loopback only and requires a per-install bearer token, and a WebX
...
WebX uses HyperDHT via Hyperswarm.
...
**Dormant** — (1) Of a WXIP: entered automatically after 365 days without substantive activity;
...
**Helia** — The JavaScript IPFS implementation WebX targets for content storage and retrieval.
...
**Hyperswarm** — The peer discovery and connection layer used to find other WebX nodes.

## I

**Impossibility-and-capture analysis** — A mandatory WXIP section (Article 5) answering two
questions: what does this change make impossible, and what new thing does it make worth
capturing? WebX prefers impossibilities to policies because an impossibility cannot be petitioned.

**IPFS** — InterPlanetary File System. The content-addressed storage network WebX publishes to.

**IPNS** — InterPlanetary Name System. A mutable pointer to a CID, signed by its owner. In WebX it
is one record type among several, not the naming layer — WebX's naming layer is the registry.

## L

**Label** — The part of a name before the extension: `example` in `example.webx`. One to 63
...
withdrawal by any single party would degrade the system. Every WXIP must analyse itself against
it.
...
**Objection Register** — The permanent public record attached to every WXIP holding each
...
**Proof-of-work** — In WebX, a memory-hard cost imposed at registration and renewal to price bulk
...
**Radicle** — The peer-to-peer code collaboration network that will become WebX's long-term
...
**Resolver proxy** — The local process on `127.0.0.1:7654` that makes WebX names work in an
unmodified browser. It never falls through to clearnet DNS for a WebX name and does not log
queries by default.

**Rights-impact analysis** — A mandatory WXIP section naming every Title II Article a change
...
tightened: WebX additionally requires that every objection and its answer be permanently recorded.
...
**Sybil attack** — Creating many identities to gain disproportionate influence. WebX resists it in
...
**Tauri** — The desktop application framework (2.x) targeted for the WebX client.
...
**WXIP** — WebX Improvement Proposal. The numbered, archived, permanently public unit of change.
Defined by [WXIP-0000](spec/WXIP-0000.md).

**WXIP editor** — A holder of a ministerial role: checks completeness, assigns numbers, records
