...
with 404 or `fallback` with 200. Of the publish flow in section 1, steps 2, 3 and 5 — build the
tree, address it, sign the pointer — now exist in code: the desktop client builds the UnixFS DAG
and root CID in `client/src/publish.rs`, pinned against this implementation's importer through
`conformance/client-built.json`. The authoring checks of section 3, local pinning (step 4) and
the content-security profile's publish-time enforcement do not exist in any implementation, and
the flow is wired together end to end nowhere; a Draft stays a Draft until someone can run it.
