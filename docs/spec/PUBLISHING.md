...
locally, and its output is READABLE BY NAME, RENEWABLE, AND LEAVABLE: `vayu publish` checks,
builds,
...
`vayu import` maintain and EXCHANGE that view — an append-only log whose per-name state is
derived by replay, answering NAME_TAKEN before any cryptography, resolving a transfer's
authority from the chain itself, and syncing between machines as one canonical CBOR bundle in
which every incoming record is judged before it enters; and `vayu serve --name`
