...
|---|---|
| **Name control** | The ability to keep answering for `example.webx`. Losing it is losing an identity that others link to. |
...
| **Registry integrity** | Every peer's ability to independently verify the whole history. If this breaks, WebX is a slow database with extra steps. |
...
| **The governance record** | The archive of what was decided and what was objected to. An unauditable record is the condition WebX exists to end. |
...
operator** is the most likely path by which WebX would quietly re-centralise, and it requires no
...
victim who is watching. WebX does not defend a machine the attacker already owns.
...
WebX. That is a fair trade to lose people over, not one to soften.
...
*Impact:* a de facto root, exactly what WebX exists to remove.
...
publicly.
*Residual risk:* **Moderate, and the most likely way WebX fails.** Convenience defaults are how
...
*Impact:* WebX unreachable for those users.
*Mitigation:* none within WebX.
*Residual risk:* **Total, and explicitly out of scope.** WebX removes intermediaries from naming
...
guaranteed. WebX makes no uptime promise because no participant is in a position to make one.
...
something WebX can fix at the naming layer. Article 24 refuses to claim otherwise.
...
*Impact:* WebX becomes what it replaced, while appearing procedurally correct throughout.
...
**T17 — Trademark or legal seizure of the name "WebX".**
*Vector:* a third party registers the mark and asserts it; or a court orders a transfer.
*Impact:* the project loses the ability to call itself WebX.
...
*Impact:* WebX calcifies around early mistakes and is displaced by something worse but newer.
...
- **WebX does not promise availability.** Content lives while someone pins it, and not one moment
  longer.
- **WebX cannot forget.** An append-only log cannot unlearn, and no peer can be compelled to
  discard bytes it already holds.
- **WebX does not resist a state at the network layer.** Blocking, seizure and compulsion are all
  outside what a naming protocol can address.
- **WebX will not adjudicate.** Not trademarks, not truth, not merit, not who deserves a name.

## See also

- [The WebX Constitution](../constitution/CONSTITUTION.md) — Title III states the limits as
  binding obligations
