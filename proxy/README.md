...
`.webx` and its sibling extensions resolve in an ordinary browser, with no extension required.
...
- Resolve a WebX name against the local registry index, select the applicable record, and
...
What it MUST NOT do: fall through to clearnet DNS for a WebX name, log queries by default,
phone home, or let a WebX page reach the control API.
