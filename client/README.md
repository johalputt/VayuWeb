...
expired predecessor. The publish path exists end to end AND RUNS: `src/doctor.rs` runs the
authoring checks first (every finding names file, line, what breaks, why, and the fix, in
...
signing. The `vayu` binary exposes it headlessly — `vayu doctor <dir>` renders every finding
with its rule and remedy; `vayu publish <dir> --name label.tld --key-file seed.hex` checks,
builds, pins, signs and prints the record; republishing under `--prev` is an UPDATE. And the
result is readable: `vayu serve <store> --root <cid>` answers a browser on loopback from a
verified traversal (`src/dagnode.rs`), carrying PUBLISHING.md's routing rule and
CONTENT-SECURITY.md's header set byte for byte. What does not exist yet is anything that makes a
stranger a reader: no resolver resolves names across machines, block exchange needs a transport
that does not exist, and the GUI is Phase 5.
...
unbuilt is everything below Tauri: read-time enforcement of the doctor's rules, serving pinned
blocks to peers, pin-set management UI, browsing, and the window itself.
