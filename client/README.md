...
expired predecessor. And the content side of publishing exists end to end as a library seam:
`src/cid.rs` addresses bytes (CIDv1 raw/dag-pb, base32 multibase), `src/publish.rs` builds the
UnixFS DAG, `src/store.rs` pins those blocks locally with verification on both write and read,
and `src/publish_flow.rs` chains all of it so that pinning structurally happens before signing —
the step ordering PUBLISHING.md makes load-bearing. No command runs any of it yet; the surface
is what a future CLI verb or GUI button will call.
...
unbuilt is everything below Tauri: the authoring checks, a command surface to run the publish
flow, serving pinned blocks to peers, pin-set management UI, browsing, and the window itself.
