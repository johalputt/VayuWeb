...
expired predecessor. And the content side of publishing exists too: `src/cid.rs` addresses bytes
(CIDv1 raw/dag-pb, base32 multibase), `src/publish.rs` builds the UnixFS DAG — so a site
directory becomes the root CID a record points at — and `src/store.rs` pins those blocks locally,
verifying every block against its CID on the way in AND on the way out, because disk corruption
and a hostile writer are the same problem: bytes that do not match their address.
...
unbuilt is everything below Tauri: serving pinned blocks to peers, the authoring checks, pin-set
management UI, browsing, and the window itself.
