...
(CIDv1 raw/dag-pb, base32 multibase) and `src/publish.rs` builds the UnixFS DAG — so a site
directory can become the root CID a record points at, with every layout pinned against blocks
the reference IPFS importer produced.
...
unbuilt is everything below Tauri: local pinning of built blocks, the authoring checks, pin-set
management, browsing, and the window itself.
