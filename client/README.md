...
builds, pins, signs and prints the record; republishing under `--prev` is an UPDATE. What does
not exist yet is anything that serves the published result to a reader: no resolver reads these
trees, block exchange needs a transport that does not exist, and the GUI is Phase 5.
